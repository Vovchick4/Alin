...
export default function ReservACar(
    ...
    const formik = useFormik<any>({
        initialValues: {
            locationFrom: cities.find(({ id }: any) => id === "1"),
            locationTo: cities.find(({ id }: any) => id === "1"),
            name: '',
            email: '',
            phone: '',
            comment: ''
        ...
        onSubmit: (values) => {
            if (fDate.date >= tDate.date) {
                Alert.alert(
                    "Invalid dates range!",
                    t("Reservation can't be retroactive"),
                    [
                        {
                            text: 'OK'
                        }
            ...
            const data = {
                ...
                services: isCheckedBounces.map((item: any) => item.name + " " + item.price),
                ...values
            }
    ...
    useEffect(() => {
        ...
        priceDepositDays()
        setTotalPrice((prev: number) => countDays * prev + totalPriceServices)
        ...
        if (moment(fTime.date).format('HH:mm') >= '21:00' ||
            ...
            setTotalPrice((prev: number) => prev + 10)
        ...
        if (formik.values.locationFrom !== formik.values.locationTo) {
            setTotalPrice((prev: number) => prev + 100)
            ...
            if (formik.values.locationTo) {
                if (formik.values.locationTo.only_return == 1) {
                    setTotalPrice((prev: number) => prev + 120)
    ...
    function getWeeksDay(date: any) { // метод провіряє який сьогодні день тижня потрібний для провірки нероблчого дня (Неділя)
        ...
        <ScrollView style={styles.contentForm}>
            ...
            <View style={styles.form}>
                ...
                <View style={styles.flexRow}>
                    <Select
                        data={cities}
                        filterOptions={({ only_return }: any) => only_return != 1}
                        renderOptions={({ title }: any) => title}
                        renderValues={(item: any) => item}
                        selectedValue={formik.values.locationFrom}
                        onChange={(itemValue: any) => formik.setFieldValue("locationFrom", itemValue)}
                    ...
                    <Select
                        ...
                        onChange={(itemValue: any) => formik.setFieldValue("locationTo", itemValue)}
